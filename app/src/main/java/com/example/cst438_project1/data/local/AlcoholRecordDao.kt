...
interface AlcoholRecordDao {
    ...
    suspend fun getLastConsumedDate(
        userId: Int,
        alcId: String
    ): String?
    ...
    @Query(
        ...
        SELECT
            ...
            record.date AS date,
            experience.rating AS rating,
            experience.user_review AS user_review
        FROM Alcohol_Record AS record
        INNER JOIN Alcohol AS alcohol
            ON alcohol.id = record.alc_id
        LEFT JOIN Alcohol_Experience AS experience
            ON experience.user_id = record.user_id
            AND experience.alc_id = record.alc_id
